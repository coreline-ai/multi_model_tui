...
import type { ClaudeCliResponse } from "./types.js";

export class ClaudeCliTransport implements ProviderTransport {
  ...
  getCapabilities(): ProviderCapabilities {
    return {
      ...
      features: {
        ...
        systemInstruction: { platform: "partial", transport: "partial", effective: "partial" },
        ...
        structuredOutput: { platform: "partial", transport: "none", effective: "none" },
  ...
  async checkAvailability(): Promise<{ ok: boolean; reason?: string }> {
    return (await commandExists("claude")) ? { ok: true } : { ok: false, reason: "claude CLI not found" };
  }
  ...
  async execute(req: UnifiedRequest): Promise<UnifiedResponse> {
    ...
    return {
      ...
      text: payload.result,
      parts: makeTextResponseParts(payload.result),
      finishReason: "stop",
    };
  }
